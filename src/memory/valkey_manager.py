...
class ValkeyManager:
    ...
    def get_session_summary(self, doc_id: str) -> Dict[str, Any]:
        """
        ...
        """
        session = self.get_session(doc_id)
        term_mappings = self.get_all_term_mappings(doc_id)
        ...
        for term, mapping in term_mappings.items():
            ...
            if mapping.locked:
                locked_terms_list.append(term_info)
            else:
                unlocked_terms_list.append(term_info)

        return {
            'session_id': doc_id,
            'status': session.status if session else 'unknown',
            'created_at': session.created_at.isoformat() if session and hasattr(session.created_at, 'isoformat') else None,
            'updated_at': session.updated_at.isoformat() if session and hasattr(session.updated_at, 'isoformat') else None,
            'source_language': session.source_language if session else None,
            'target_language': session.target_language if session else None,
            'total_segments': session.total_segments if session else 0,
            'processed_segments': session.processed_segments if session else 0,
            'term_count': session.term_count if session else 0,
            'total_term_mappings': len(term_mappings),
            'locked_terms_count': len(locked_terms_list),
            'unlocked_terms_count': len(unlocked_terms_list),
            'locked_terms': locked_terms_list,
            'unlocked_terms': unlocked_terms_list,
        }
