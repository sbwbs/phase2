...
def check_context_builder():
    """Test context builder without external dependencies"""
    ...
    try:
        ...
        if result.optimized_context:
            print(f"✅ Context builder working: {result.total_tokens} tokens generated")
            print(f"   Components included: {len(result.components_included)}")
            return True
        else:
            print("⚠️  Context builder produced empty result")
            return False
