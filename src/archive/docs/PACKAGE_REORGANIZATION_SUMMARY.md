...
**Before**: Files used relative imports (`from .context_builder import`) without proper package structure, causing import errors when running scripts directly.
...
- **enhanced_translation_service.py**: Changed `from .context_builder` → `from context_builder`
- **context_builder.py**: Changed `from .token_optimizer` → `from token_optimizer`
...
- `/Users/won.suh/Project/translate-ai/phase2/src/context_builder.py` (imports fixed)
